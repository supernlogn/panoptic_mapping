...
DriftGenerator::DriftGenerator(const ros::NodeHandle& nh, 
  ...
  noise_file_output_(initializeStreamFromRosParams(DriftGenerator::Config::fromRosParams(nh_private))),
...
void DriftGenerator::generate_noisy_pose_callback(const geometry_msgs::TransformStamped& msg) {
  ...
  if(config_.save_to_file) {
    ...
    noise_file_output_ << "\"rotation\""<< ": [" << msg.transform.rotation.x << 
                                            ", " << msg.transform.rotation.y << 
                                            ", " << msg.transform.rotation.z << 
                                            ", " << msg.transform.rotation.w << 
                                            "], ";
    noise_file_output_ << "\"translation\""<< ": [" << msg.transform.translation.x << 
                                            ", " << msg.transform.translation.y << 
                                            ", " << msg.transform.translation.z <<
                                            "]";
    noise_file_output_ << "}, ";
    noise_file_output_ << "\"noisy\":{";
    noise_file_output_ << "\"rotation\""<< ": [" << msg_noisy_pose.transform.rotation.x << 
                                            ", " << msg_noisy_pose.transform.rotation.y << 
                                            ", " << msg_noisy_pose.transform.rotation.z << 
                                            ", " << msg_noisy_pose.transform.rotation.w << 
                                            "], ";
    noise_file_output_ << "\"translation\""<< ": [" << msg_noisy_pose.transform.translation.x << 
...
bool DriftGenerator::setupROS() {
  ...
  pose_sub_ = nh_.subscribe(config_.ground_truth_Pose_topic, queue_length,
...
DriftGenerator::Config DriftGenerator::Config::fromRosParams(const ros::NodeHandle& nh) {
