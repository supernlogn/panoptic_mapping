...
class VariableCountLayer : public ClassLayerImpl<VariableCountVoxel> {
 public:
  struct Config : public config_utilities::Config<Config> {
    ...
    bool use_binary_classification = true;
 ...
 * generally used to store the belonging submap and shifting other IDs by 1.
 */
struct MultiVariableCountVoxel : public ClassVoxel {
 public:
  // Implement interfaces.
  ClassVoxelType getVoxelType() const override;
  bool isObserverd() const override;
  ...
  ClassificationCount current_count = 0;
  ClassificationCount total_count = 0;
  ClassificationCount classes_count = 0;
  static std::unordered_set<int> submap_class;
};

class MultiVariableCountLayer : public ClassLayerImpl<MultiVariableCountVoxel> {
 public:
  struct Config : public config_utilities::Config<Config> {
    Config() {
      std::unordered_set<int> submap_classes;
      setConfigName("MultiVariableCountLayer");
    }
    bool use_binary_classification = true;
  ...
  MultiVariableCountLayer(const Config& config, const float voxel_size,
                          const int voxels_per_side);
 ...
 protected:
  const Config config_;
  static config_utilities::Factory::RegistrationRos<
      ClassLayer, MultiVariableCountLayer, float, int>
      registration_;
};
