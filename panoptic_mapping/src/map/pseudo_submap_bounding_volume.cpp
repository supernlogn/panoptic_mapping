...
PseudoSubmapBoundingVolume::PseudoSubmapBoundingVolume(const PseudoSubmap& submap)
    : submap_(&submap),
      center_(0.f, 0.f, 0.f),
      radius_(0.f),
...
PseudoSubmapBoundingVolume::PseudoSubmapBoundingVolume(const PseudoSubmap& submap, 
    ...
    : submap_(&submap),
      center_(submapBoundingVolume.center_),
      radius_(submapBoundingVolume.radius_),
      num_previous_blocks_(submapBoundingVolume.num_previous_blocks_) {}

void PseudoSubmapBoundingVolume::clone(PseudoSubmapBoundingVolume & other) const {
  other.submap_ = submap_;
  other.center_ = center_;
  other.radius_ = radius_;
  other.num_previous_blocks_ = num_previous_blocks_;
}

void PseudoSubmapBoundingVolume::update() {
...
bool PseudoSubmapBoundingVolume::contains_S(const Point& point_S) const {
  // Point is expected in submap frame.
  return (center_ - point_S).norm() <= radius_;
}

bool PseudoSubmapBoundingVolume::contains_M(const Point& point_M) const {
  return contains_S(submap_->getT_S_M() * point_M);
}

bool PseudoSubmapBoundingVolume::intersects(const PseudoSubmapBoundingVolume& other) const {
...
bool PseudoSubmapBoundingVolume::isInsidePlane_S(const Point& normal_S) const {
  // The normal is expected in submap frame.
  return center_.dot(normal_S) >= -radius_;
}

bool PseudoSubmapBoundingVolume::isInsidePlane_M(const Point& normal_S) const {
