...
bool VariableCountVoxel::belongsToSubmap() const {
  ...
  // voxels.
  return current_index == submap_class;
}

float VariableCountVoxel::getBelongingProbability() const {
  return getProbability(submap_class);
}
...
// MultiVariable Class Voxel

ClassVoxelType MultiVariableCountVoxel::getVoxelType() const {
  return ClassVoxelType::kMultiVariableCount;
}

bool MultiVariableCountVoxel::isObserverd() const { return !counts.empty(); }

bool MultiVariableCountVoxel::belongsToSubmap() const {
  // In doubt we count the voxel as belonging. This also applies for unobserved
  // voxels.
  return classes_count >= current_count;
  // return true;
}

float MultiVariableCountVoxel::getBelongingProbability() const {
  float f = 0.0;
  for (int cid : submap_class) {
    f += getProbability(cid);
  }
  return f;
}

int MultiVariableCountVoxel::getBelongingID() const { return current_index; }

float MultiVariableCountVoxel::getProbability(const int id) const {
  if (counts.empty()) {
    return 0.f;
  }
  auto it = counts.find(id);
...
void MultiVariableCountVoxel::incrementCount(const int id, const float weight) {
  const ClassificationCount new_count = ++(counts[id]);
  if (submap_class.count(id) > 0) {
    classes_count++;
  }
  if (new_count > current_count) {
    current_index = id;
...
bool MultiVariableCountVoxel::mergeVoxel(const ClassVoxel& other) {
  // Check type compatibility.
  auto voxel = dynamic_cast<const MultiVariableCountVoxel*>(&other);
  if (!voxel) {
    LOG(WARNING) << "Can not merge voxels that are not of same type "
                    "(MultiVariableCountVoxel).";
    return false;
  }
  classes_count += voxel->classes_count;
...
std::vector<uint32_t> MultiVariableCountVoxel::serializeVoxelToInt() const {
  // To save memory, here we just assume that the IDs stored in the map are in
  // int_16 range (-32k:32k).
  std::vector<uint32_t> result(counts.size() + 1);
  result[0] = counts.size();
...
bool MultiVariableCountVoxel::deseriliazeVoxelFromInt(
    const std::vector<uint32_t>& data, size_t* data_index) {
  if (*data_index >= data.size()) {
    LOG(WARNING)
        << "Can not deserialize voxel from integer data: Out of range (index: "
  ...
  *data_index += length;
  classes_count = 0;
  for (int cid : submap_class) {
    if (counts.count(cid) > 0) {
      classes_count += counts.at(cid);
    }
  }
  return true;
}

config_utilities::Factory::RegistrationRos<ClassLayer, MultiVariableCountLayer,
                                           float, int>
    MultiVariableCountLayer::registration_("multivariable_count");

MultiVariableCountLayer::MultiVariableCountLayer(const Config& config,
                                                 const float voxel_size,
                                                 const int voxels_per_side)
    : config_(config.checkValid()),
      ClassLayerImpl(voxel_size, voxels_per_side) {}

ClassVoxelType MultiVariableCountLayer::getVoxelType() const {
  return ClassVoxelType::kMultiVariableCount;
}

std::unique_ptr<ClassLayer> MultiVariableCountLayer::clone() const {
  return std::make_unique<MultiVariableCountLayer>(*this);
}

std::unique_ptr<ClassLayer> MultiVariableCountLayer::loadFromStream(
    const SubmapProto& submap_proto, std::istream* /* proto_file_ptr */,
    uint64_t* /* tmp_byte_offset_ptr */) {
  // Nothing special needed to configure for binary counts.
  return std::make_unique<MultiVariableCountLayer>(
      MultiVariableCountLayer::Config(), submap_proto.voxel_size(),
      submap_proto.voxels_per_side());
}

std::unordered_set<int> MultiVariableCountVoxel::submap_class = {0, 1, 2,
                                                                 3, 4, 5};
